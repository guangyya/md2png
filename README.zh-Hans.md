...
结果很长时，可改用“将剪贴板分片保存为 PNG…”。选择一个上级文件夹后，md2png
会新建文件夹并保存带编号的 PNG；这个显式导出只读取当前剪贴板，不会替换它。
...
| 将剪贴板分片保存为 PNG… | — | 将剪贴板 Markdown 保存到新文件夹中的多张编号 PNG，不改变剪贴板 |
| 显示上次渲染 | `Control-Command-Z`（全局） | 打开最近结果，并提供复制、保存、在“预览”中打开、适合窗口、实际大小和缩放操作 |
| 重新渲染上次的 Markdown | — | 使用当前主题和宽度重新渲染最近一次成功的源 Markdown |
| 恢复上次的 Markdown | — | 将最近一次成功的源 Markdown 恢复到剪贴板 |
| 主题 | — | 为后续渲染选择简洁浅色、暖色纸张或深色，并在本地记住选择 |
| 输出宽度 | — | 为后续渲染选择紧凑、标准或宽幅，并在本地记住选择 |
| 示例 | — | 复制并渲染选中的内置示例，然后自动打开预览 |
...
“宽幅”则为大型表格和图表留出更多空间。“简洁浅色”是默认主题；“暖色纸张”和
“深色”会统一切换 Markdown、代码高亮和 Mermaid 配色，但不会改变字体、间距或
输出宽度。所选主题会固定写入不透明的 PNG。正在渲染时，新的渲染命令、宽度或主题
切换和示例会暂时禁用，避免重复渲染。只有成功渲染后才会启用“重新渲染上次的
Markdown”和“恢复上次的 Markdown”；如果其他应用已经修改剪贴板，覆盖前会要求确认。
...
采用固定补零编号，绝不覆盖已有文件夹。
“上次渲染”窗口会在当前屏幕范围内按输出宽度打开，并在标题中标明所用预设和
实际尺寸。工具栏可以再次复制图片、明确保存 PNG、在“预览”中打开、适合窗口、
按显示器像素查看实际大小，以及缩放预览；这些操作不会改变生成的图片或剪贴板内容。
