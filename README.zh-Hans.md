...
结果很长时，可改用“将剪贴板分片保存为 PNG…”。分片渲染完成后，文件夹选择器会
显示准确的 PNG 数量。选择一个上级文件夹后，md2png 会新建文件夹并保存带编号的
PNG；这个显式导出只读取当前剪贴板，不会替换它。保存完成后可选择“在 Finder 中
显示”，一次选中全部导出图片并拖到其他应用。
...
| 渲染 Markdown 文件… | — | 读取明确选择的 UTF-8 `.md`、`.markdown` 或 `.txt` 文件，仅在成功后将 PNG/TIFF 写入剪贴板 |
